...
// which is what a reactive visualizer needs. Returns how far above the
// rolling average the current hit is (`ratio`) so the caller can drive a
// proportional pulse instead of a flat on/off flash.
export class BeatDetector {
  constructor({ historySize = 40, threshold = 1.28, minGapMs = 110 } = {}) {
    this.history = [];
  ...
  update(energy, nowMs) {
    ...
    const ratio = avg > 0.001 ? energy / avg : 1;
    let isBeat = false;
    if (
